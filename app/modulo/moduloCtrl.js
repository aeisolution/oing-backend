...
(function () {
	'use strict';
	var controllerId = 'moduloCtrl';
	angular.module('app').controller(controllerId, ['webconfig', 'dataFactory', '$window', '$scope', '$filter', '$modal', 'toastr', moduloCtrl]);

	function moduloCtrl(webconfig, dataFactory, $window, $scope, $filter, $modal, toastr) {
		var vm = this;
		vm.title = 'Archivio Documenti e Moduli';
		vm.view = 'elenco';
		vm.elenco = [];
		vm.page = 1;
		vm.numRecords = 0;
		...
		//****************************************************
		function getPage() {
			dataFactory.baseGetPage('moduli', vm.page).then(function (data) {
		...
		function getRecord(id) {
			return dataFactory.baseGetById('moduli', id).then(function (data) {
				vm.record = data.data;				
			});
		}
		...
		function postRecord() {
			dataFactory.basePost('moduli',vm.record)
				.then(
					function (data) {
						getPage();
						switchView();
		...
		function putRecord() {
			dataFactory.basePut('moduli', vm.record._id, vm.record).then(function (data) {
		...
		function deleteRecord(item) {
			...
			dataFactory.baseDelete('moduli', item._id).then(function (data) {
	...
	//ALLEGATI
		function sendFile(file) {
			...
			dataFactory.uploadAllegato('moduli', vm.record._id, file)
				.then(
					function (data) { 
		...
		function allegatoSave(item) {
			dataFactory.baseAllegatiPut('moduli', vm.record._id, item)
				.then(					
					function (data) { 
		...
		function allegatoDelete(item) {
			...
			modalInstance.result
				.then(
					function () { 
						dataFactory.baseAllegatiDelete('moduli', vm.record._id, item.id)
							.then(					
								function (data) { 
